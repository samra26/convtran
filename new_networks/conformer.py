...
class Decoder(nn.Module):
    ...
    def forward(self, lde_c,gde_c,lde_t,gde_t,q,k,v):
        ...
        for k in range(len(gde_c)):
            ...
            high_features_tran.append(torch.cat((gde_t[k][1]*(self.softmax(q[h_index][1].squeeze(1)*k[h_index][0].squeeze(1))*v[h_index][0].squeeze(1)),lde_t[h_index][0]*(self.softmax(q[h_index][0].squeeze(1)*k[h_index][1].squeeze(1))*v[h_index][1].squeeze(1))),dim=1))
            h_index=h_index+1
